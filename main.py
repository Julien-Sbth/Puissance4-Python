...
class InterfaceGraphique:
    def __init__(self):
        ...
        self.ia1 = IA()
        self.ia2 = IA()  # Deux instances d'IA
        self.ia1.charger_donnees()
        self.ia2.charger_donnees()

        self.tour = 1
        self.mode = None
        self.coups_joueur1 = []  # Liste pour enregistrer les coups du joueur 1
        self.coups_joueur2 = []  # Liste pour enregistrer les coups du joueur 2
        ...
        self.btn_ia_vs_ia = tk.Button(self.fen, text="IA contre IA", command=self.ia_vs_ia_action)  # Bouton pour IA vs IA
    ...
    def jouer_contre_ia_action(self):
        ...
        self.mode = "ia"
        self.tour = 1
        self.ia1.entrainer_IA()  # Entraîner l'IA lorsque l'utilisateur choisit de jouer contre elle
    ...
    def ia_vs_ia_action(self):
        ...
        # Démarrer la boucle principale pour faire jouer les IA automatiquement
        ...
        if self.mode == "joueur":
            if self.tour == 1:
                ...
                if self.jeu.check_victoire():
                    ...
                    self.ia1.sauvegarder_donnees()  # Sauvegarder le coup joué par le joueur 1
                    return
                self.tour = 2
            else:
                self.jeu.jouer(colonne)
                ...
                if self.jeu.check_victoire():
                    ...
                    self.ia1.enregistrer_resultat(self.jeu.plateau)
                    self.ia1.sauvegarder_donnees()  # Sauvegarder le coup joué par le joueur 2
                    return
                self.tour = 1
        elif self.mode == "ia":
            if self.tour == 1:
                ...
                if self.jeu.check_victoire():
                    ...
                    self.ia.enregistrer_resultat(self.jeu.plateau)  # Sauvegarder le coup joué par l'IA
    ...
    def ia_joue(self, ia):
        if ia == self.ia1:  # Vérifier le rôle de l'IA
            role = self.ia1_role
        else:
            role = self.ia2_role

        if role == "joueur1":  # Si c'est le tour de l'IA de jouer en tant que joueur 1
            ...
            if hauteur_colonne is not None:
                ...
                if self.jeu.check_victoire():
                    ...
                    return
                self.tour = 2
        else:  # Si c'est le tour de l'IA de jouer en tant que joueur 2
            coup_ia = ia.prendre_decision_intelligente(self.jeu.plateau)
            hauteur_colonne = next((i for i, row in enumerate(self.jeu.plateau) if row[coup_ia] == 0), None)
            if hauteur_colonne is not None:
                self.jeu.jouer(coup_ia)
